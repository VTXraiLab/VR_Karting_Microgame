...
public class GameFlowManager : MonoBehaviour
{
    ...
    void EndGame(bool win)
    {
        ...
        if (win)
        {
            ...
            int i;
            int numPlaces = 3;
            float playTime = m_TimeManager.TotalTime - m_TimeManager.TimeRemaining;

            string sceneName = PlayerPrefs.GetString("CurrentSceneName");
            float topScore = float.PositiveInfinity;
            string topName = PlayerPrefs.GetString("PlayerName");
            string scoreKey = "";

            for (i = 1; i <= numPlaces; i++)
            {
                scoreKey = $"{sceneName}_{i}_score";
                topScore = PlayerPrefs.GetFloat(scoreKey, float.PositiveInfinity);

                if (topScore > playTime)
                {
                    topScore = playTime;
                    break;
                }
            ...
            while (i <= numPlaces)
            {
                scoreKey = $"{sceneName}_{i}";

                tempScore = PlayerPrefs.GetFloat($"{scoreKey}_score", float.PositiveInfinity);
                tempName = PlayerPrefs.GetString($"{scoreKey}_name", "NONE");
                PlayerPrefs.SetFloat($"{scoreKey}_score", topScore);
                PlayerPrefs.SetString($"{scoreKey}_name", topName);
